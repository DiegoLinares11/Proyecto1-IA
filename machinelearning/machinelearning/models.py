...
class PerceptronModel(Module):
    def __init__(self, dimensions):
        ...
        wtensor = torch.ones(1, dimensions)

        self.w = Parameter(wtensor)
    ...
    def train(self, dataset):
        ...
        statement = True
        while statement:
            # Assume we haven't seen 100% accuracy yet
            errors = 0
            for batch in dataloader:
                # Get the features and labels from the batch
                x = batch['x']
                label = batch['label']
                ...
                if prediction != label:
                    # Misclassified sample, update the weights
                    with torch.no_grad():  # Temporarily disable gradient tracking
                        self.w += label * x.squeeze()
                    errors += 1
            
            # If there are no errors, stop training (100% accuracy)
            if errors == 0:
